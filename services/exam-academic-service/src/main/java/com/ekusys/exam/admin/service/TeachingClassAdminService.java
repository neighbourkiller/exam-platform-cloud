...
public class TeachingClassAdminService {
    ...
    public List<TeachingClassView> toTeachingClassViews(List<TeachingClass> classes) {
        ...
        Set<Long> teacherIds = classes.stream()
            ...
            .filter(Objects::nonNull)
            .collect(Collectors.toSet());
        Map<Long, UserSummary> teacherMap = teacherIds.isEmpty()
            ? Collections.emptyMap()
            : iamUserClient.batch(new UserBatchRequest(teacherIds.stream().toList())).getData().stream()
                .collect(Collectors.toMap(UserSummary::id, item -> item, (a, b) -> a));
        ...
        return classes.stream()
            ...
            .map(tc -> {
                ...
                UserSummary teacher = teacherMap.get(tc.getTeacherId());
                return TeachingClassView.builder()
                    ...
                    .teacherName(teacher == null ? null : teacher.realName())
                    .term(tc.getTerm())
                    .status(tc.getStatus())
    ...
    public void ensureTeachingClassRelation(Long subjectId, Long teacherId) {
        ...
        UserSummary teacher = iamUserClient.get(teacherId).getData();
        if (teacher == null) {
            throw new BusinessException("教师用户不存在");
        }
        if (!teacher.roles().contains(ROLE_TEACHER)) {
