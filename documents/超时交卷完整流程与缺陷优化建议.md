...
2. **V2 的关键 MySQL 并发测试当前失败。** 本次实际执行 `TimeoutSubmissionV2MySqlTest`，100 次并发重复交卷出现死锁；两个领取者并发使用 `SKIP LOCKED` 时，一方领取到 0 条而不是预期的 10 条。
3. **前端把“服务器已接管”误当成“Runtime 已完成交卷”。** 收到 `SUBMITTING` 后会立即清除本地草稿、停止计时并跳到成绩页；如果后台任务之后重试耗尽并进入 `FAILED`，学生侧已经丢失本地恢复与取证入口。
...
综合判断：**V2 应继续保持关闭，先修复本文 P0 项并让并发集成测试稳定通过；V1 可以作为兼容路径继续运行，但其持久化重试、失败可见性、最终载荷完整性和容量保障明显弱于 V2，不能作为长期目标方案。**
...
| `PREPARED` | 考试终止 | `CANCELLED` | 当前实现只取消尚未激活的会话 |
...
### T-00：V2 默认未启用，开启时必须保证全部 Runtime 配置一致（P0 上线前置）
...
2. 在 `docker-compose.yml` 显式透传 `APP_TIMEOUT_SUBMISSION_V2_ENABLED`，并保持默认 `false`；
3. 启动时输出非敏感的最终开关和关键容量参数；增加所有 Runtime 实例配置一致性检查；
4. 发布时暂停 XXL-Job，统一升级全部实例、核对缺失任务为 0 后再统一开关，禁止长期混跑。

### T-01：V2 并发重复手工交卷出现真实 MySQL 死锁（P0，已证实）
...
### T-02：`SKIP LOCKED` 未实现预期的双领取者并行分配（P0，已证实）
...
PENDING 查询按 `due_at, id` 排序，但现有索引是 `(status, due_at, next_retry_at, id)`，`next_retry_at` 位于排序列中间，加上 `(next_retry_at IS NULL OR next_retry_at <= now)` 条件，可能触发 filesort/扩大锁定扫描范围。结果是一个事务为了返回 10 条候选，可能锁住更多符合条件的行，另一个 `SKIP LOCKED` 领取者看不到剩余任务。

该索引执行计划推断还需要用与生产数据分布相同的 `EXPLAIN ANALYZE` 和 InnoDB 锁观测确认，但当前集成测试已证明“两个领取者均匀领取”的设计假设不成立。
...
### T-03：前端在 `SUBMITTING` 时过早清理本地草稿并离页（P0）
...
### T-04：截止瞬间存在“快照已接受但未进入最终答案”的竞态（P1）
...
### T-06：任务在工作池排队期间租约已开始消耗，且无续租（P1）
...
领取事务会一次性给整批任务写入 60 秒租约，再把全部任务提交到 8 线程工作池。队尾任务需要等待前面多轮任务完成；如果平均单任务处理超过约 2.4 秒，队尾在开始前就可能接近或超过租约。当前没有 lease heartbeat/renew。
...
### T-07：`max-run-ms` 不是严格运行上限（P1）

`processDue` 只在领取下一批前检查 `System.nanoTime()`，随后对整批 `CompletableFuture.allOf(...).join()` 无期限等待。任一 Redis/数据库调用长期阻塞时：
...
### T-08：`FAILED` 后 Session 永久停留在 `AUTO_SUBMITTING`（P1）

失败处理只把 Task 改为 `FAILED`，不会改变 Session。当前安全重放依赖运维人员暂停 XXL-Job 后执行 SQL。
...
### T-09：提前终止考试只取消 PREPARED，会放过正在作答的会话（P1）

`ExamProvisioningService.terminate` 只处理：
...
### T-12：超时标记和状态语义没有完整投影（P2）

- `SubmissionAccepted` 已包含 `timeoutSubmit`，但 Reporting 的 `projectSubmission` 没有保存该字段；
...
| --- | --- | --- |
| P0 | T-01 并发重复交卷死锁 | 是 |
| P0 | T-02 SKIP LOCKED 并行领取失败 | 是 |
| P0 | T-03 SUBMITTING 误清草稿/误离页 | 是 |
| P0 | T-00 全实例统一开关与 Compose 透传 | 是 |
| P1 | T-04 截止快照竞态 | 是，考试答案完整性风险 |
| P1 | T-05 时间源不统一 | 是，截止边界风险 |
| P1 | T-06/T-07 租约排队与无界运行 | 大规模考试前必须修复 |
| P1 | T-08 永久失败业务闭环 | 大规模考试前必须修复 |
| P1 | T-09 提前终止语义 | 取决于产品语义，但必须明确 |
| P2 | T-10～T-13 体验、投影、监控和测试 | 上线前至少完成关键部分 |
