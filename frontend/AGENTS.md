...
- 修改运行代码、依赖或构建配置时，在本目录运行 `npm run build`；纯文档修改检查内容、链接和差异即可。
- 可用验证以 `package.json` 的 scripts 为准，按变更选择相关测试；不得声称执行了不存在或未运行的验证。
- 修改交互流程时，对受影响角色和关键流程执行手工冒烟；无法运行时，说明原因和待验证场景。
