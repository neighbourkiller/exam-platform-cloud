...
本文件适用于 `deploy/` 下的启动脚本、Nacos 配置、Secrets 准备、压测工具和运维手册，并与仓库根 `AGENTS.md` 共同生效。
...
- `deploy/scripts/` 中的宿主机部署脚本只允许作为调用 `docker-compose.yml` 的示例，不得生成配置、发布 Nacos、写数据库、单独启动中间件或在宿主机构建 JAR。
...
- `deploy/load-test/` 按 `scenarios/`、`harness/`、`compose/`、`fixtures/` 和本地 `results/` 分层；其中 Compose 文件只作为唯一根 Compose 入口的测试覆盖文件。
- 不在本文件复述完整启动步骤、端口或基础设施版本；修改部署流程时，仅更新 README 和 `deploy/runbooks/` 中受影响的说明。
...
- Outbox 上线、观察、重放和清理遵循 [`runbooks/OUTBOX_OPERATIONS.md`](runbooks/OUTBOX_OPERATIONS.md)；快照增量落库排障遵循 [`runbooks/SNAPSHOT_FLUSH_OPERATIONS.md`](runbooks/SNAPSHOT_FLUSH_OPERATIONS.md)。
